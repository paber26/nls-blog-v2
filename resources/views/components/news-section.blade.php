    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
      <div class="flex flex-col md:flex-row md:items-end justify-between mb-12 gap-6">
        <div class="max-w-2xl">
          ...
          <p class="text-lg text-slate-600">Temukan info beasiswa, tips belajar, strategi lomba, dan artikel persiapan akademik yang bisa kamu baca kapan saja.</p>
        </div>
        <a href="/articleList"
          Lihat Semua Artikel
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        <article
          ...
          :key="article.id"
          class="bg-white rounded-3xl overflow-hidden shadow-xl shadow-slate-200/40 border border-slate-100 group flex flex-col hover:-translate-y-2 transition-all duration-300"
        >
          ...
          <div class="p-8 flex flex-col flex-grow">
            <h3 class="text-xl font-bold font-heading text-slate-800 mb-3 group-hover:text-brand-light transition-colors">{{ article.title }}</h3>
            <p class="text-slate-600 mb-6 text-sm line-clamp-3">{{ article.summary }}</p>
            <div class="mt-auto flex items-center justify-between text-sm text-slate-500">
              ...
              <RouterLink :to="{ name: 'articleDetail', params: { id: article.id } }" class="text-brand-light font-semibold hover:underline">Baca Selengkapnya</a>
            </div>
          </div>
        </article>
      </div>
